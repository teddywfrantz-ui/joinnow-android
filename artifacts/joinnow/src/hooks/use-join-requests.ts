...
export function useJoinRequests(meetupId?: number) {
      ...
      if (message.meetupId === meetupId) {
        switch (message.type) {
          case 'join_request':
            ...
            queryClient.invalidateQueries({ queryKey: [`/api/meetups/${meetupId}/requests`] });
            queryClient.invalidateQueries({ queryKey: ['/api/meetups'] });
            queryClient.invalidateQueries({ queryKey: ['/api/notifications'] });

            // Force a refetch of requests
            queryClient.refetchQueries({ queryKey: [`/api/meetups/${meetupId}/requests`] });
            break;
          ...
          case 'request_update':
            ...
            queryClient.invalidateQueries({ queryKey: [`/api/meetups/${meetupId}/requests`] });
            queryClient.invalidateQueries({ queryKey: [`/api/meetups/${meetupId}/my-request`] });
            queryClient.invalidateQueries({ queryKey: ['/api/meetups'] });
            queryClient.invalidateQueries({ queryKey: ['/api/notifications'] });

            // Force refetch of critical data
            queryClient.refetchQueries({ queryKey: [`/api/meetups/${meetupId}/requests`] });
            queryClient.refetchQueries({ queryKey: [`/api/meetups/${meetupId}/my-request`] });
            break;
  ...
  const sendRequest = useMutation({
    mutationFn: async ({ meetupId, message }: { meetupId: number; message?: string }) => {
      const res = await fetch(`/api/meetups/${meetupId}/join`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({ message: message || "I'd like to join your meet!" }),
        credentials: 'include'
      });
  ...
  const cancelRequest = async (requestId: number) => {
    ...
    try {
      ...
      const previousRequest = queryClient.getQueryData<JoinRequest>([`/api/meetups/${meetupId}/my-request`]);
      const previousRequests = queryClient.getQueryData<JoinRequest[]>([`/api/meetups/${meetupId}/requests`]);
      ...
      queryClient.setQueriesData({ queryKey: ['/api/meetups'] }, (old: any) => {
        ...
        return old.map(m => 
          m.id === meetupId 
            ? { ...m, pendingRequestCount: Math.max(0, (m.pendingRequestCount || 0) - 1) }
            : m
        );
      });
      ...
      if (!res.ok) {
        ...
        queryClient.setQueryData([`/api/meetups/${meetupId}/my-request`], previousRequest);
        if (previousRequests) {
          queryClient.setQueryData([`/api/meetups/${meetupId}/requests`], previousRequests);
        }
      ...
      console.log('Request cancelled successfully:', {
        meetupId,
        requestId,
        newMyRequest: queryClient.getQueryData([`/api/meetups/${meetupId}/my-request`])
      });
  ...
  const handleRequest = async ({ requestId, meetupId, status }: { requestId: number, meetupId: number, status: 'accepted' | 'rejected' }) => {
    try {
      const res = await fetch(`/api/meetups/${meetupId}/requests/${requestId}`, {
        method: 'POST',
      ...
      sendMessage({
        ...
        message: status === 'accepted' ? 'Your request to join has been accepted!' : 'Your request to join has been rejected.'
      });
  ...
  const leaveMeetup = async () => {
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: [`/api/meetups/${meetupId}/participants`] });
      await queryClient.invalidateQueries({ queryKey: [`/api/meetups/${meetupId}/my-request`] });
      await queryClient.invalidateQueries({ queryKey: ['/api/notifications'] });

      toast({
        title: "Left meetup",
        description: "You have successfully left the meetup.",
      });
    } catch (error: any) {
      toast({
