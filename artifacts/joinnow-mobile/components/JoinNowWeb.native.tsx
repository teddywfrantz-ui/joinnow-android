...
export function JoinNowWeb() {
  ...
  const isAuthPath = ['/auth', '/login', '/register'].some((path) =>
    currentPath.startsWith(path),
  );
  const hideNavigation = keyboardVisible || isAuthPath;
  const iosNavigationHeight = 64 + insets.bottom;
  const reserveIOSNavigation = Platform.OS === 'ios' && !isAuthPath;
    ...
    >
      <KeyboardAvoidingView
        ...
        style={[
          ...
          reserveIOSNavigation && { marginBottom: iosNavigationHeight },
        ]}
      >
        <WebView
          ref={webView}
      ...
      {!hideNavigation ? (
        <>
          ...
          <View
            style={[
              styles.bottomNav,
              Platform.OS === 'ios' && styles.bottomNavOverlay,
              {
                backgroundColor: colors.card,
                borderColor: colors.border,
                paddingBottom: insets.bottom,
